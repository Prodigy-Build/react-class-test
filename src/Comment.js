...
const Comment = ({ id, level, loadingSpinner, threadStore }) => {
  ...
  const firebaseRef = useFirebase().database().ref(`items/${id}`);
  ...
  useEffect(() => {
    const handleFirebaseRefCancelled = (e) => {
      ...
      setComment((prevComment) => {
        if (!prevComment.delayed) {
          return { ...prevComment, delayed: true };
        }
        return prevComment;
      });
