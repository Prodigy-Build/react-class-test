...
const Stories = (props) => {
    ...
    const handleUpdate = (update) => {
      if (!isMounted) {
        if (process.env.NODE_ENV !== 'production') {
          console.warn(`Skipping update as the ${props.type} Stories component is no longer mounted.`);
        }
        return;
      }
  ...
  const getPageNumber = () => {
    // implement getPageNumber logic here
  };
  ...
  if (ids == null) {
    ...
    for (let i = page.startIndex; i < page.endIndex; i++) {
      dummyItems.push(<li key={i} className="ListItem ListItem--loading" style={{ marginBottom: SettingsStore.listSpacing }}>
        <Spinner />
      </li>);
    }
    return (
      <div className="Items Items--loading">
        ...
        <Paginator route={props.route} page={page.pageNum} hasNext={page.hasNext} />
      </div>
    );
  }
