...
function Item(props) {
  ...
  useEffect(() => {
    ...
    const itemListener = firebase.database().ref(itemRef).on('value', (snapshot) => {
      const itemData = snapshot.val();
      setItem(itemData);
    });

    return () => {
      firebase.database().ref(itemRef).off('value', itemListener);
    };
  }, [props.params.id, firebase]);

  useEffect(() => {
    if (item.id) {
      const threadStore = new StoryCommentThreadStore(item, handleCommentsChanged, { cached: true });
  ...
  useEffect(() => {
    if (props.params.id !== nextProps.params.id) {
      ...
      firebase.database().ref(HNService.itemRef(nextProps.params.id)).once('value', (snapshot) => {
        const itemData = snapshot.val();
        if (itemData) {
          const threadStore = new StoryCommentThreadStore(itemData, handleCommentsChanged, { cached: true });
          setThreadStore(threadStore);
  ...
  useEffect(() => {
    if (prevState.item.id !== item.id) {
      if (!threadStore || threadStore.itemId !== item.id) {
        const threadStore = new StoryCommentThreadStore(item, handleCommentsChanged, { cached: false });
        setThreadStore(threadStore);
        setTitle(item.title);
        forceUpdate();
      }
